...
H = 40  #Using 40 neurons in hidden layer

for i in range(len(C_list)):#C_list:
...
# TEST
yp_test = predict(X_test, Win, V)
y_test = y_test.reshape((y_test.shape[0], 1))
E_test = np.sum((y_test - yp_test)**2)
E_test /= y_test.shape[0]

if test:
    plt.figure()
    plt.plot(y_test, 'g')
    plt.plot(yp_test, 'r')
    plt.title('Expected and Predicted outputs')
    plt.ylabel('Output')
    plt.xlabel('samples')
    ...
    print('Error rate in test dataset with ', H, ' neurons in hidden layer ans C = ', C_list[18], ': ', E_test)
    ...
    plt.plot(train_err, 'g')
    plt.plot(val_err, 'r')
    plt.title('Least Mean Square Error - '+str(H)+' neurons in hidden layer')
    plt.ylabel('error')
